...
class TestCreateFlow:
    ...
    async def test_create_flow_without_project(self, tenant_id, flow):
        flow_id = await prefect.api.flows.create_flow(
            serialized_flow=flow.serialize(), tenant_id=tenant_id
        )
        assert await models.Flow.where(id=flow_id).first()
